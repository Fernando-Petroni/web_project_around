...
popupAddForm.addEventListener("click", (event) => {
  if (event.target.classList.contains("popup-add-card")) {
    popupAddForm.classList.remove("popup_visible");
  }
});

modalImage.addEventListener("click", (event) => {
  if (event.target.classList.contains("imagebox")) {
    modalImage.classList.remove("popup_visible");
  }
});

// mensagem de error

// const inputs = document.forms["formeditPopup"].querySelectorAll(".popup__form");
